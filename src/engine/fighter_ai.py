    ...
    def __init__(self, state=None, data=None):
        ...
        Args:
            state: The current AI's state.
                ...
                Typically a string but can be any data type needed.
...
stats - An iterable of stats to check. If None, defaults to user.stats."""
        ...
        for stat in stats:
            if f'{stat}Cost' in move:
                ...
                if getattr(user, f'user.{stat}_bound').right == 0:
                    return False
                ...
                cost *= util.divi_zero(
                    ...
                    getattr(user, f'user.{stat}_bound').right,
                    True, -cls.moveCostAverage(move, stat) * 100)
        ...
        for stat in stats:
            if f'{stat}Cost' in move:
                ...
                if isinstance(move[f'{stat}Cost'], Bound):
                    if (
                            (stat_max := getattr(
                                user, f'user.{stat}_bound').right)
                            < move[f'{stat}Cost'].left
                            ):
                        logger.debug(f"""\
...
stat - The stat of the value. Used in algorithm to decide computations."""
        if stat == 'hp':
            hp, hp_max = user.hp, user.hp_bound.right
            ...
            #       #
            #1# # # # # # # # # # # max - stat
            weight = util.divi_zero(10, hp_max * (hpRatio / 25) ** 4) + 1
            ...
            st, st_max = user.st, user.st_bound.right
            stRatio = (1 - st / st_max) * 100
            # Ratio Graph
            # 4.85                #
            #                    #
            #                   #
            ...
            #     #
            #1# # # # # # # # # # # max - stat
            weight = util.divi_zero(10, st_max * (stRatio / 25) ** 2.8) + 1
            weight *= self.data[f'{stat}ValueBias']
            logger.debug(
            ...
            mp, mp_max = user.mp, user.mp_bound.right
            mpRatio = (1 - mp / mp_max) * 100
            # Ratio Graph
            # 4.85                #
            #                    #
            #                   #
            ...
            #     #
            #1# # # # # # # # # # # max - stat
            weight = util.divi_zero(10, mp_max * (mpRatio / 25) ** 2.8) + 1
            weight *= self.data[f'{stat}ValueBias']
            logger.debug(
...
Picked counter {randomCounter!r}""")
        ...
        for counter in counters:
            ...
            counter_damage_reduction *= util.divi_zero(
                user.hp_bound.right - user.hp, 25)
        ...
        for counter in counters:
            ...
            average_chance_exclusive = util.divi_zero(
                totals['cChance'] - counterChance,
                len_counters - 1
            ) / 100
...
{debugWeight}""")
    ...
    def make_weights_relative(iterable):
        ...
        weights = [
            util.divi_zero(w, weightSum) for w in iterable]
        return weights
...
class FighterAIFootsies(FighterAIGeneric):
    ...
    def analyseMove(self, user, target):
        ...
        energy_percent = int(user.st / user.st_bound.right * 100)
        target_health_float = target.hp / target.hp_bound.right
        ...
        target_energy_float = target.st / target.st_bound.right
        ...
        low_on_health = user.hp < user.hp_bound.right
        health_percent = int(user.hp / user.hp_bound.right * 100)
